...
public class GDConfigGen extends ConfigDataProvider {
	...
	public void add(ConfigDataProvider.Collector map) {
		map.add(GolemDungeons.SPAWN, GolemDungeons.loc("factory_remnant"), new SpawnConfig()
				...
				.type(GolemTypes.TYPE_DOG.get(), new SpawnConfig.GolemTypeEntry(30, 0)
						.add(GolemItems.DIAMOND.get(), 0.7f)
				)
				...
				.upgrade(GolemItems.SIZE_UPGRADE.asItem(), 30)
				...
				.equipments(new SpawnConfig.EquipmentGroup(GolemTypes.ENTITY_HUMANOID.get())
						.add(new SpawnConfig.EquipmentSet(100)
								...
								.add(EquipmentSlot.OFFHAND, 100, getRocket(), 0)
						)
				)
	...
	private static ItemStack getRocket() {
		var ans = Items.FIREWORK_ROCKET.getDefaultInstance();
		FireworkRocketItem.setDuration(ans, (byte) 3);
		return ans;
	}

	private static SpawnConfig.EquipmentSet tippedSet(int weight) {
	...
	private static ItemStack tipped(Potion potion) {
